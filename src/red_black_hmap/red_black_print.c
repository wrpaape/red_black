#include "red_black_tree/red_black_print.h"    /* stddef -> size_t, NULL */
...
size_red_black_node(const struct RedBlackHNode *const restrict node,
...
static size_t
size_red_black_tree(const struct RedBlackHNode *const restrict node,
		    const RedBlackKeySizer key_sizer,
		    const unsigned int indent)
{
	if (node == NULL)
...
put_red_black_node(char *restrict buffer,
		   const struct RedBlackHNode *const restrict node,
...
bool
red_black_print(const struct RedBlackHNode *const restrict root,
		const RedBlackKeySizer key_sizer,
		const RedBlackKeyPutter key_putter)
{
	bool success;
