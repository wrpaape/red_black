	     ...
	     const void *const key)
{
	...
	if (status) {
		parent->right = rb_new_node(allocator,
					    jump_buffer,
					    key,
					    true); /* RED */

		/* need to correct */
		rb_insert_correct_rr_bot(tree,
					 grandparent,
					 parent,
					 jump_buffer);
